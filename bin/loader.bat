...
if not [%XIL_PA_NO_XILINX_OVERRIDE%] == [1] (
  rem #
  rem # On 64bit hosts look for 64bit XILINX tools first then fall back 32bit.
  rem #
  if defined XILINX (
    if [%RDI_OS_ARCH%] == [64] (
      if [%RDI_PLATFORM%] == [win64] (
  ...
  if defined XILINX_SDK (
    if [%RDI_OS_ARCH%] == [64] (
      if [%RDI_PLATFORM%] == [win64] (
        ...
        set _valid=True
        for %%d in (lib/nt, bin/nt) do (
          if not exist "%XILINX_SDK%/%%d" (
            set _valid=False
          )
        )
        if [!_valid!] == [True] (
          set RDI_SDK_PLATFORM=nt
        )
      )
    ) else (
      set _valid=True
...
if [%_RDI_NEEDS_PYTHON%] == [True] (
  set RDI_PYTHON2_VERSION=2.7.5
  set RDI_PYTHON2=%RDI_APPROOT%\tps\%RDI_PLATFORM%\python-!RDI_PYTHON2_VERSION!
  set RDI_PYTHONHOME=!RDI_PYTHON2!
  set RDI_PYTHONPATH=!RDI_PYTHONHOME!;!RDI_PYTHONHOME!\lib;!RDI_PYTHONHOME!\Lib\lib-tk;!RDI_PYTHONHOME!\lib\site-packages
  set RDI_PYTHON_LD_LIBPATH=!RDI_PYTHONHOME!\lib

  if exist "!RDI_PYTHON2!" (
    set PYTHON=!RDI_PYTHON2!
