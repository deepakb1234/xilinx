...
IF "%XBATCHCFG%"=="" (
  ...
  %X_JAVA_HOME%\bin\java.exe -splash:%parent%\data\images\splash.png %ARGS% -cp %X_CLASS_PATH% -jar %parent%\lib\classes\xinstaller.jar
) ELSE (
  echo "XBATCHXFG is set, forcing batch mode"
  %X_JAVA_HOME%\bin\java.exe %ARGS% -cp %X_CLASS_PATH%;%parent%\lib\classes\commons-cli-1.2.jar com.xilinx.installer.cli.InstallerCLI -a %XBATCHACTION% -c %XBATCHCFG%
)
